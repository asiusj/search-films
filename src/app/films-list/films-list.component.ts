...
export class FilmsListComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.location.path() === '/search') {
      ...
      this.fs.getStoredFilmsObs().subscribe(
        r => {
          this.films = r;
        }
      );
  ...
  saveFilm(f: Film) {
    ...
    this.fs.addFilmToLocalStorage(f).subscribe(
      r => this.storedFilms = r
    );
  }

  deleteFilm(f: Film) {
    this.fs.removeFilmFromLocalStorage(f.StorageId).subscribe(
      r => {
        this.storedFilms = r;
