...
export class FilmService {
  ...
  getFilmsList(s: string): Observable<Film[]> {
    if (s.length > 2) {
      return this.http.get<Film[]>(
        environment.apiUrl + "?s=" + s + "&apikey=e290de4d&r=json&v=2"
      );
    } else if (s.length === 0) {
      ...
      return this.http.get<Film[]>(
        environment.apiUrl + "?s=" + random + "&apikey=e290de4d&r=json&v=2"
      );
    }
  }
  ...
  getFilmById(id: string): Observable<Film> {
    return this.http.get<Film>(
      environment.apiUrl + "?i=" + id + "&apikey=e290de4d&r=json&v=2"
    );
  }
